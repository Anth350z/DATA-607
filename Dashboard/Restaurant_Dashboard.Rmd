...
desc <- df
df <- as.data.frame(df[1:180,]) %>% select(BORO,ZIPCODE,`CUISINE DESCRIPTION`,`VIOLATION CODE`,`CRITICAL FLAG`,SCORE,GRADE, `INSPECTION DATE`)  %>% drop_na() 
...
plot.1 <- df.new %>% mutate( year = format(`INSPECTION DATE`, "%Y")) %>%
    group_by(year) %>%
        summarise(total = n()) %>%
          plot_ly(x=~ year , 
                  y = ~total , 
...
```
> This how many violations inspections date happened by Year counts.
...
plot.1 <- df.new %>% mutate( month = format(`INSPECTION DATE`, "%B")) %>%
    group_by(month,BORO) %>%
        summarise(total = n()) %>%
          plot_ly(x=~ month , 
                  y = ~ BORO , 
